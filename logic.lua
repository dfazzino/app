...
function checkLogic(ids, trueactions, falseactions, delete)
	...
	for i, logicID in ipairs(idList) do
		...
		local response
		if operation == "is" or operation == ">" or operation == "<" then
			response = mathLogic(operation, operand1, operand2)
		end	
		if operation == "at" then
			response = entityLogic(aLogic.ifString)
		end		
		if operation == "has" or operation == "notHas" then
			response = inventoryLogic(operation, operand1, operand2)
		end
...
function inventoryLogic (operation, name, itemID)
