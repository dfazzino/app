...
function loadEntities (entitiesSection)

	logFile:write("Starting to add entities!\n")

	for line in entitiesSection:lines() do
...
function moveToEntity(entity, toEntity, action)
	for _, iL in ipairs(toEntity.interactLocations) do
		if iL.action == action then
			local ixy  = iL.xy:split(",")
			local xy  = toEntity.xy:split(",")
			entity.walkingToEntity = toEntity
			return	pathFinding("move" .. " " .. entity.name .. " " .. math.floor((xy[1] + ixy[1]) / 30) .. "," .. math.floor((xy[2] + ixy[2]) / 30 ), "_" .. entity.name .. "_" .. toEntity.name ..  "_", true)	
		end
	end
end

function tryToFindPath(entityName, toEntityName)
